...
class VNLinear(nn.Module):
    ...
    def forward(self, x):
        '''
        x: point features of shape [B, in_channels, 3, npoint, ...]
        --------
        x_out: point features of shape [B, out_channels, 3, npoint, ...]
        '''
...
class VNLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        '''
        x, x_out: point features of shape [B, in_channels, 3, npoint, ...]
        '''
        d = self.map_to_dir(x.transpose(1,-1)).transpose(1,-1)  #[B, in_channels or 1, 3, npoint, ...]
...
class VNStdFeatureLin(nn.Module):
    ...
    def forward(self, x):
        ...
        --------
        z0: [B, 3, 3, npoint, ...]
        x_std: [B, N_feat, 3, npoint, ...]
        '''
        z0 = x
        z0 = self.vn_lin(z0.transpose(1, -1)).transpose(1, -1)  #(B, 3, 3, npoint, nsample)
...
class VNStdFeatureRe(nn.Module):
    def __init__(self, in_channels, dim=4, normalize_frame=False, share_nonlinearity=False, negative_slope=0.2):
        super(VNStdFeatureRe, self).__init__()
        self.dim = dim
        self.normalize_frame = normalize_frame
        self.vn1 = VNLinearAndLeakyReLU(in_channels, in_channels//2, dim, share_nonlinearity, 'none', negative_slope)
        self.vn2 = VNLinearAndLeakyReLU(in_channels//2, in_channels//4, dim, share_nonlinearity, 'none', negative_slope)
        if normalize_frame:
